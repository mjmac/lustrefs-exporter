...
| `--extended-client-metrics` | `LUSTREFS_EXPORTER_EXTENDED_CLIENT_METRICS` | Also export `lustre_client_osc_rpc_stats_bytes_per_rpc` and `lustre_client_mdc_rpc_stats_bytes_per_rpc`: the pages-per-RPC table with every bucket bound multiplied by this kernel's page size, so RPC sizes compare across clients with different page sizes. The variable takes the same values as `--aggregate-client-metrics`. |

`lustre_page_size_bytes`, the page size of the kernel the exporter runs under, is exported on
every scrape.
